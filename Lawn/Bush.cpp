...
void Bush::BushInitialize(int theRow, bool theNight)
{
    ...
    if (theNight)
        aIndex += 3;
    mPosX = (mBoard->StageHas6Rows() ? cBushPos6Rows[theRow][0] : cBushPos[theRow][0]) - (BOARD_OFFSET_X / 2);
    mPosY = (mBoard->StageHas6Rows() ? cBushPos6Rows[theRow][1] : cBushPos[theRow][1]) - 60;
    mID = theRow;
    mBushIndex = aIndex;
