	...
	Dialog::BUTTONS_FOOTER)
{
    ...
    {
        mDialogLines = _S("[CONTINUE_GAME_OR_RESTART]");
        mContinueButton = MakeButton(ContinueDialog::ContinueDialog_Continue, this, _S("[CONTINUE_BUTTON]"));
        mNewGameButton = MakeButton(ContinueDialog::ContinueDialog_NewGame, this, _S("[RESTART_LEVEL_BUTTON]"));
    }
    else
    {
        mDialogLines = _S("[CONTINUE_GAME]");
        mContinueButton = MakeButton(ContinueDialog::ContinueDialog_Continue, this, _S("[CONTINUE_BUTTON]"));
...
void ContinueDialog::ButtonDepress(int theId)
{
    ...
    else if (theId == ContinueDialog::ContinueDialog_NewGame)
    {
        ...
        else
        {
            ...
            aDialog->mLawnYesButton->mLabel = _S("[NEW_GAME_BUTTON]");
            //aDialog->CalcSize(0, 0);
        }
    }
    else
